    ...
    <div class="padding">
        ...
        <div class="table-responsive">
            <table class="table table-input">
                ...
                <tbody>
                    @foreach ($components as $mainComponent)
                        <tr>
                            <td>{{ $mainComponent->key_ar }}</td>
                            <form action="{{ route('dashboard.main-components.update', $mainComponent) }}" method="POST"
                                ...
                                <td>
                                    @if ($mainComponent->isFile)
                                        <div class="border border-secondary p-2">
                                            ...
                                            <img id="imgPreview"
                                                src="{{ App\Helpers\Upload::getImgUrl($mainComponent->value_en) }}"
                                                alt="" width="100%">
                                        </div>
                                    @else
                                    <div>
                                        <textarea name="value_ar" class="form-control" disabled>{{ $mainComponent->value_ar }}</textarea>
                                        <textarea name="value_en" class="form-control" disabled>{{ $mainComponent->value_en }}</textarea>
                                    </div>
                                    @endif
                                </td>
