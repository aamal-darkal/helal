...
@section('content')
    ...
    <form action="{{ route('dashboard.provinces.update', $province) }}" method="post">
        ...
        <x-input name="location_ar" label="العنوان بالعربي" required maxlength="255"  :dbValue="$province->location_ar" />
        <x-input name="location_en" label="العنوان بالانكليزي " required maxlength="255" :dbValue="$province->location_en"/>
